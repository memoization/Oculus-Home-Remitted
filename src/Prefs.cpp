...
unsigned Prefs::SaveTick()
{
    return prefs.g_saveTick.load(std::memory_order_relaxed);
}

std::wstring Prefs::Widen(const std::string& utf8)
{
...
std::string Prefs::Narrow(const std::wstring& wide)
{
...
{
    return prefs.AppDir() + L"preferences.json";
}
...
std::string Prefs::GetDisplayName()
{
    std::string text = ReadFileUtf8(PrefsPath());
...
void Prefs::SetDisplayName(const std::string& name)
{
    std::string text = ReadFileUtf8(PrefsPath());
    std::string err;
    json11::Json existing = text.empty() ? json11::Json() : json11::Json::parse(text, err);
...
std::string Prefs::GetProfileImagePath()
{
    std::string text = ReadFileUtf8(PrefsPath());
    if (text.empty()) return std::string();
    std::string err;
...
void Prefs::SetProfileImagePath(const std::string& path)
{
    std::string text = ReadFileUtf8(PrefsPath());
    std::string err;
    json11::Json existing = text.empty() ? json11::Json() : json11::Json::parse(text, err);
...
std::string Prefs::GetDefaultWorldId()
{
    std::string text = ReadFileUtf8(PrefsPath());
    if (text.empty()) return std::string();
    std::string err;
...
void Prefs::SetDefaultWorldId(const std::string& id)
{
...
std::vector<std::string> Prefs::GetOculusLibraryPaths()
{
...
void Prefs::SetOculusLibraryPaths(const std::vector<std::string>& paths)
{
...
Prefs::SetCaptureFlags Prefs::GetCaptureFlags()
{
    std::string text = ReadFileUtf8(PrefsPath());
